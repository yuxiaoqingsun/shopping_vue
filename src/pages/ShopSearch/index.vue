...
export default {
  ...
  mounted() {
    this.getData();
    // ...mapActions('shopsearch',['getSearchInfo'])
  },
  computed: {
    ...mapGetters("shopsearch", ["goodsList"]),
    ...mapState("shopsearch",{
  ...
  methods: {
    getData() {
      // ...mapActions('shopsearch')
      this.$store.dispatch("shopsearch/getSearchInfo", this.searchParams);
      // console.log(this.searchParams)
    },
    removeCategoryName() {
      // 因为这些参数是可有可无的，如果设为空字符串还是会带给服务器，可以设置为undefined
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      this.getData();
      //如果typeNav中id用的判断写法，在这里要把上次的id置空
      //因为if方法只会带一个id到query参数，这样合并的时候不会把原有的和此次不同的id覆盖掉
      //而没用if方法，会把所有id都传到query参数，也就是会把和此次不同的id的undefined值带过去，合并的时候就会把原有的和此次不同的id置为undefined
      //如果用if在这里置空
    },
  },
};
